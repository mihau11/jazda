...
# --- GAME LOGIC CLASS (Modified to return a summary dictionary) ---
class GameLogic:
    def __init__(self):
    ...
    def get_daily_consumption(self):
        ...
        return {key: int(val * random.uniform(0.9, 1.1)) for key, val in consumption.items()}
    ...
    def resolve_day(self):
        ...
        if self.current_stance == 'Defensive':
            ...
            if is_failed:
                outcome = "Offensive Failed"
                integrity_change -= 25; self.offensive_days_counter = 0
                ...
                outcome = "Offensive Succeeded"
                self.offensive_days_counter += 1; self.frontline_distance += 1
                losses = {k: random.randint(int(getattr(self, f"start_{k}")*m1), int(getattr(self, f"start_{k}")*m2)) for k, m1, m2 in 
        ...
        return {"outcome": outcome, "losses": losses, "consumption": daily_needs, "integrity_change": self.frontline_integrity - initial_integrity}
    ...
    def dispatch_convoy(self, convoy_type, cargo):
        ...
        if convoy_type == 'Train': self.rail_cooldown = 3
...
class DailySummaryWindow(tk.Toplevel):
    def __init__(self, parent, summary):
        ...
        self.geometry("400x350"); self.resizable(False, False); self.transient(parent); self.grab_set()
...
class DispatchWindow(tk.Toplevel):
    ...
    def __init__(self, parent, game_logic, convoy_type):
        super().__init__(parent); self.parent = parent; self.game_logic = game_logic; self.convoy_type = convoy_type
        self.capacity = 250 if convoy_type == 'Trucks' else 1000
        self.title(f"Dispatch {convoy_type}"); self.geometry("400x350"); self.resizable(False, False); self.transient(parent)
        self.sliders = {}; self.slider_vars = {}
    ...
    def send(self):
        ...
        self.parent.update_display(); self.destroy()

class ReinforcementWindow(tk.Toplevel):
    # ... (Unchanged)
    def __init__(self, parent, game_logic):
        super().__init__(parent); self.parent = parent; self.game_logic = game_logic
        self.title("Reinforcements Available"); self.geometry("450x200"); self.resizable(False, False); self.transient(parent)
    ...
    def choose(self, choice):
        self.game_logic.apply_reinforcements(choice); self.parent.update_display(); self.destroy()

# --- MAIN GUI APPLICATION ---
class LogisticsCommandGUI(tk.Tk):
    def __init__(self):
        super().__init__(); self.title("Logistics Command: After Action Report Edition")
    ...
    def create_widgets(self):
        ...
        ttk.Label(left_frame, text="Depot Supplies", font=('Helvetica', 12, 'bold')).pack(pady=10)
        ...
        self.filter_buttons = {}
        for cat in ["Total", "Fuel", "Rations", "Rifle Ammo", "Gun Ammo"]:
            btn = tk.Button(graph_filter_frame, text=cat, command=lambda c=cat: self.set_graph_filter(c))
            btn.pack(side='left', padx=3); self.filter_buttons[cat] = btn
        ...
        # --- MODIFIED: Action buttons are now at the bottom of the right panel ---
        action_frame_bottom = tk.Frame(right_frame); action_frame_bottom.pack(side='bottom', pady=5)
        ttk.Separator(action_frame_bottom, orient='horizontal').pack(fill='x', pady=5)
        convoy_status_frame = tk.Frame(action_frame_bottom); convoy_status_frame.pack(side='top', pady=5)
        ...
        main_actions_frame = tk.Frame(action_frame_bottom); main_actions_frame.pack(side='top', pady=5)
        stance_frame = tk.Frame(main_actions_frame); stance_frame.pack(side='left', padx=10)
        self.stance_def_btn = tk.Button(stance_frame, text="Set Defensive", command=lambda: self.set_stance('Defensive')); self.stance_def_btn.pack(pady=2)
        self.stance_off_btn = tk.Button(stance_frame, text="Set Offensive", command=lambda: self.set_stance('Offensive')); self.stance_off_btn.pack(pady=2)
        convoy_frame = tk.Frame(main_actions_frame); convoy_frame.pack(side='left', padx=10)
        self.truck_btn = tk.Button(convoy_frame, text="Dispatch Trucks", command=lambda: self.open_dispatch_window('Trucks')); self.truck_btn.pack(pady=2)
        self.train_btn = tk.Button(convoy_frame, text="Dispatch Train", command=lambda: self.open_dispatch_window('Train')); self.train_btn.pack(pady=2)
        
        self.end_day_btn = tk.Button(main_actions_frame, text="END DAY", font=('Helvetica', 16, 'bold'), bg='darkgreen', fg='white', command=self.end_day_click)
        self.end_day_btn.pack(side='left', fill='both', padx=20, ipadx=10)
        self.set_graph_filter("Total")

    def set_stance(self, stance):
        self.game_logic.current_stance = stance; self.update_display()

    def set_graph_filter(self, category):
        self.graph_category_var.set(category)
    ...
    def end_day_click(self):
        ...
        if self.game_logic.game_over:
            ...
            self.update_display() # Update display again after reinforcements
    ...
    def update_unit_dots(self):
        ...
        for unit, canvas in self.unit_canvases.items():
            ...
            for _ in range(dot_count):
                canvas.create_rectangle(x, y, x + dot_size, y + dot_size, fill='black', outline="")
                x += dot_size + 2
    ...
    def update_display(self):
        gl = self.game_logic
        self.day_var.set(f"DAY: {gl.day}"); self.stance_var.set(f"STANCE: {gl.current_stance}"); self.integrity_bar['value'] = gl.frontline_integrity
